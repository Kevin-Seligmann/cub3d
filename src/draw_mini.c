...
void	draw_mini_player(t_ged *ged, t_sim *sim)
{
	...
	while (square_x < MM_SCALE)
	{
		...
		{
			mlx_put_pixel(ged->minimap, sim->player.pos.x * MM_SCALE + square_x - MM_SCALE / 2, sim->player.pos.z * MM_SCALE + square_z - MM_SCALE / 2, 0xFF0000FF);
			square_z++;
		}
		square_x++;
	}		
...
void	draw_square_1(t_ged *ged, unsigned int x, unsigned int y)
{
	...
	while (square_x < MM_SCALE)
	{
		...
		while (square_y < MM_SCALE)
		{
			mlx_put_pixel(ged->minimap, x * MM_SCALE + square_x, y * MM_SCALE + square_y, 0x0000FFFF);
			square_y++;
		}
		square_x++;
	}
...
/*Draw the minimap (walls) in Scale proportion of the map, using MM_SCale as multiplier and a colour */
void	draw_square_2(t_ged *ged, unsigned int x, unsigned int y)
{
	...
	while (square_x < MM_SCALE)
	{
		...
		while (square_y < MM_SCALE)
		{
			mlx_put_pixel(ged->minimap, x * MM_SCALE + square_x, y * MM_SCALE + square_y, 0xFFFFFF0);
			square_y++;
		}
		square_x++;
	}
